# Incidence Disease Comorbidity Analysis

The **Incidence Disease Comorbidity Analysis** followed a similar design to the **Baseline Disease Comorbidity Analysis**, with one key difference:  
- Chapter 15 contained **no comorbid cases** for incident diseases and was therefore **excluded** from the analysis.

As a result, the only methodological difference lies in the **Bonferroni correction factor**, which was adjusted to:  
`14 chapters × 251 metabolic traits`

## Statistical Framework

- **Binary comorbidity outcome**:  
  Individuals diagnosed with **two or more incident diseases** within a given chapter were considered **cases**, while those with **fewer than two** were treated as **controls**.  
  Associations with metabolites were tested using **logistic regression**.

- **Comorbidity count analysis**:  
  Among individuals with at least one incident disease, we used **ordinal logistic regression** to assess the relationship between metabolite levels and the **number of incident diseases**.
...
- Both **logistic regression** and **ordinal logistic regression** were implemented using the `statsmodels` package in **Python**.
- **Multiple testing correction** was applied using the **Bonferroni method**, with the adjusted correction factor:  
  `14 chapters × 251 metabolic traits`
- A corrected **p-value threshold of 0.05** was used to determine statistical significance.
...
data_folder = "data"
cluster_file = 'Results/s5_Chapter'
...
disease_df = pd.read_csv(os.path.join(root_path, cluster_file, 'data/disease_data_chapter_in.csv'))
cluster_cols = [col for col in disease_df.columns if col.startswith('Chapter_')]
...
nb_cpus = 10
save_dir = os.path.join(root_path, 'Results/s5_Chapter/Association/in')
