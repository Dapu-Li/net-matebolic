# Baseline Disease Comorbidity Analysis

The baseline **comorbidity analysis** was divided into two parts:

1. **Association between metabolites and comorbidity presence within each ICD-10 chapter**  
2. **Association between metabolites and the number of comorbid diseases**, among individuals with at least one diagnosis

## Definition of Comorbidity

- For the binary comorbidity outcome, individuals with **two or more diseases** were classified as **cases**, while those with **fewer than two diseases** were treated as **controls**.  
  Logistic regression was used to assess the association between metabolites and comorbidity presence.

- For the **number of comorbid diseases**, analyses were restricted to individuals with at least one diagnosis.  
  Associations between metabolites and comorbidity burden (i.e., number of diseases) were tested using **ordinal logistic regression**.
...
- Both **logistic regression** and **ordinal logistic regression** models were implemented using the `statsmodels` package in **Python**.
- **Multiple testing correction** was applied using the **Bonferroni method**, with a correction factor of:  
  `15 chapters × 251 metabolic traits`
- Statistical significance was determined at a corrected threshold of **p < 0.05**.
...
disease_df = pd.read_csv(os.path.join(root_path, cluster_file, 'data/disease_data_chapter_ba.csv'))
...
disease_lg.Chapter_15.value_counts()
...
nb_cpus = 10
save_dir = os.path.join(root_path, 'Results/s5_Chapter/Association/ba')
